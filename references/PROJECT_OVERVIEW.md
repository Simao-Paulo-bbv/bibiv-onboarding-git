...
Agreements_Files (the file-factory input table)
      ...
      │       for one active Job_ID at a time:
...
7. **Agreement generator is queue-backed** — AppSheet must enqueue via `generateAgreementFilesFromAppSheet`; the dispatcher creates per-file tasks for one active `Job_ID` at a time, bounded workers generate PDFs inside that active job, and the finalizer is the only step that writes `Ready` after verifying the physical PDFs exist in Drive, so mail cannot send before the complete document set exists.
